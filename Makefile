.PHONY: help build run test clean deps install uninstall system-deps update check

BINARY_NAME=radioKeenetik
VERSION=$(shell git describe --tags --always 2>/dev/null || echo "2.0.0")
...
system-deps: ## 🔧 Установить зависимости системы
	...
	sudo apt install -y \
		...
		alsa-utils pulseaudio mpv ffmpeg sox \
		libatlas-base-dev libjasper-dev libtiff5-dev \
		libharfbuzz0b libwebp6 libdc1394-25 \
		liblapack3 libopenjp2-7 fonts-dejavu-core
...
build: ## 🔨 Собрать приложение для Linux
...
build-arm: ## 🔨 Собрать для роутера ARM (Raspberry Pi, OpenWrt)
	...
	GOOS=linux GOARCH=arm GOARM=7 go build -o $(BINARY_NAME)-arm *.go
